	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Fudeo, Falhou, Faz Assim, Tenta o ".f", C Não Funcionar Reporte Ao AkTr ou ao Adryan!*',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos amigo! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *Adryan ou AkTr*',
					...
					ownerG: '[❗]  Este é um recurso especial  Hehe ❌',
					ownerB: '[❗] Este é um recurso especial Hehe ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Algum Dono. Registre-se para fazer amizade com a Bot Ana por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Ana|17\n\n──「 ANA BOT 」──`,
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("removeu voce")){
			client.updatePresence(from, Presence.composing)
			reply("*_Anti-Trollagens 1.0_*")
	}
	
	if (messagesC.includes("Removeu Você")){
			client.updatePresence(from, Presence.composing)
			reply("*_Anti-Trollagens 1.0_*")
	}
	
	if (messagesC.includes("Adicionou Mãe")){
			client.updatePresence(from, Presence.composing)
			reply("Anti-Trollagens 1.0")
	}
	...
	if (messagesC.includes("Adryan")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/Adryan.m4a');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
...
//@darkYT					
    ...
    break
				...
				case 'textblue':
					...
					break
					case 'vipmenu':
					if (!isPremium) return reply('Mano, Só O Adryan/AkTr pode Usar Este Comando Mano!' ,text, { quoted: mek })
				...
				case 'premiumlist':
					...
					break
                  case 'qrcode':
        			if (!isPremium) return reply('Mano, So O AkTr/Adryan Usa Esse Comando!' ,text, { quoted: mek })
			...
			case 'instaimg':
				...
				break
                                case 'afk':
                                        ...
                                        break 
				case 'indo10':
					if (!isPremium) return reply('Mano, So O Adryan/AkTr Usa Esse Comando!!' ,text, { quoted: mek })
...
// recursos anime, random @AkTr domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, em breve voce sera membro premium se entrar em contato con algum propietario do bot' ,text, { quoted: mek })
					...
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, em breve vc sera se entrar em contato com algum propietario do bot!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT ANA ✨* 」\n*│+ wa.me/558896668696 && wa.me/*╰──────*「 *ANA* 」*────*\n\n*_SE QUER SER ADMIN DO BOT ANA_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/55889668696 & wa.me/558197660171*╰──────*「 *ANA_BOT* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DA ANA_BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* AkTr & Adryan\n*YOUTUBE:* EM BREVE MENOR\n*WPP:* wa.me/558896668696 && wa.me/558197660171\n*INSTA:* Em Breve\n\n\nEspero que tenham gostado do bot 🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
